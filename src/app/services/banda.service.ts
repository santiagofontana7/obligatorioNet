...
export class BandaService {
  ...
  private bandasUrl = 'https://localhost:44323/api/Banda/GetAll';

  private bandas : Observable<Banda[]>;

  getBandas(): Observable<Banda[]> {
    //this.messageService.add('Servicio de bandas: bandas cargadas');
    this.bandas = this.http.get<Banda[]>(this.bandasUrl).pipe(tap(_ => this.log('bandas buscadas')), catchError(this.handleError<Banda[]>('getBandas', [])));
    return this.bandas;
  }
